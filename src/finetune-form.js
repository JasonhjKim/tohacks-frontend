...
export default class BaseForm extends Component {
    ...
    handleSubmit(e) {
        ...
        formData.append("models", this.state.base);
    ...
    handleToggleButtonChange(e) {
        // e.preventDefault();
        this.setState({base: e}, () => console.log(this.state.base))
    }

    handleRadioButtonChange(e) {
        // e.preventDefault();
        this.setState({optimizer: e}, () => console.log(this.state.optimizer))
    }

    render() {
        const base = ["VGG16", "VGG19", "ResNet50", "ResNet152", "EfficientNet", "MobileNet", "Xception"];
        const optimizer = ["adam", "adagrad", "adadelta", "adamax", "rmsprop"]
        const learningRate = [0.1, 0.05, 0.01, 0.005, 0.001, 0.0005, 0.0001, 0.00005, 0.00001]

        return(
            <StyledForm onSubmit={this.handleSubmit.bind(this)}>
                <FormTitle>Fine Tune Model: </FormTitle>
                <FormComponent>
                    <FormLabel>Please choose a model</FormLabel>
                    <ToggleButtonGroup name="test" type="radio" className="mb-2" onChange={this.handleToggleButtonChange.bind(this)}>
                        {
                ...
                <FormComponent>
                    <FormLabel>Please choose optimizer setting(s)</FormLabel>
                    <ToggleButtonGroup type="checkbox" className="mb-2" onChange={this.handleToggleButtonChange.bind(this)}>
                        {
                            optimizer.map((b, i) => <StyledToggleButton value={b}>{b}</StyledToggleButton>)
                        }
                    </ToggleButtonGroup>
                </FormComponent>

                <FormComponent>
                    <FormLabel>Please choose learning rate(s)</FormLabel>
                    <ToggleButtonGroup type="checkbox" className="mb-2" onChange={this.handleToggleButtonChange.bind(this)}>
                        {
                            learningRate.map((b, i) => <StyledToggleButton value={b}>{b}</StyledToggleButton>)
                        }
                    </ToggleButtonGroup>
                </FormComponent>
                ...
                <FormComponent>
                    ...
                    <FormGoogleDriveInput type="number" min="1" max="6" value="6"></FormGoogleDriveInput>
                </FormComponent>

                <FormComponent>
                    <FormLabel>Epoch Size</FormLabel>
                    <FormGoogleDriveInput type="number" min="1" max="6" value="5"></FormGoogleDriveInput>
                </FormComponent>

                <FormComponent>
                    <FormLabel>Validation Size</FormLabel>
                    <FormGoogleDriveInput type="number" min="0" max="1" value="0.2"></FormGoogleDriveInput>
                </FormComponent>
